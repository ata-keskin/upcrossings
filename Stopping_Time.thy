...
section \<open>Stopping Time\<close>
...
proof (rule stopping_timeI)
  fix t
  show "Measurable.pred (F t) (\<lambda>x. Inf {i. P i x} \<le> t)"
  proof cases
    assume "t = \<infinity>" 
    thus "Measurable.pred (F t) (\<lambda>\<omega>. Inf {i. P i \<omega>} \<le> t)" by auto
  next
    assume asm: "t \<noteq> \<infinity>"
    moreover
    { 
    ...
    show "Measurable.pred (F t) (\<lambda>\<omega>. Inf {i. P i \<omega>} \<le> t)" unfolding * using sets_F_mono assms by (intro pred_intros_countable_bounded) (auto simp: pred_def)
  qed
qed simp
...
section \<open>Hitting Time\<close>

text \<open>Given a stochastic process \<open>X\<close> and a borel set \<open>A\<close>, \<open>hitting_time X A n m \<omega>\<close> is the first time \<open>X\<close> is in \<open>A\<close> after time \<open>s\<close> and before time \<open>t\<close>.
      If \<open>X\<close> does not hit \<open>A\<close> after time \<open>s\<close> and before \<open>t\<close> then the hitting time is simply \<open>t\<close>.\<close>
